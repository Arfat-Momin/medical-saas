...
export const usersService = {
     ...
     *
     * Fields routed to the GLOBAL `users` table (shared by design):
     *   fullName, phone, consultationFee.
     */
    async update(auth: AuthContext, token: string, userId: string, patch: UpdateUserInput) {
        ...
        const nextMembershipActive = hasMembershipChange ? patch.isActive! : membership.is_active;
        ...
        if (Object.keys(profilePatch).length > 0) {
            after = await usersRepository.updateProfile(supabaseAdmin, userId, profilePatch);
        }
        ...
        await audit({
            ...
            after: {
                ...
                consultation_fee: after?.consultation_fee ?? null,
        ...
        return {
            ...(after ?? {}),
            // Tenant-scoped status. The frontend should read THIS field
            // to render the "Active / Inactive" badge for the current
            // tenant, not the deprecated global users.is_active.
            membership_active: nextMembershipActive,
        };
    },
};