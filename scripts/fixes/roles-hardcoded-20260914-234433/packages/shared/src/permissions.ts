...
export const DEFAULT_ROLE_PERMISSIONS: Record<RoleCode, Permission[]> = {
  ...
  /**
   * ACCOUNTANT: read access to every department whose invoice they
   * handle. Writes stay excluded — the accountant raises invoices and
   * takes payments, but does not edit clinical data.
   *
   *   - PATIENT_READ         : who is being billed
   *   - APPOINTMENT_READ     : which visit generated the doctor bill
   *   - CONSULTATION_READ    : what was prescribed / diagnosed
   *   - IPD_READ             : bed / room charges that appear on the bill
   *   - PHARMACY_READ        : dispense line items
   *   - LAB_READ             : lab order line items
   *   - BILLING_*            : the invoices themselves + payments + refunds
   */
  ACCOUNTANT: [
    PERMISSIONS.PATIENT_READ,
    PERMISSIONS.APPOINTMENT_READ,
