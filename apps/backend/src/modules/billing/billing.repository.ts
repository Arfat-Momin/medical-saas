...
export const billingRepository = {
  ...
  // ---- Invoices ----
  async listInvoices(client: SupabaseClient, tenantId: string, opts: { patientId?: string; status?: string; from?: string; to?: string; page: number; pageSize: number }) {
    const from = (opts.page - 1) * opts.pageSize;
    const to = from + opts.pageSize - 1;
  ...
  async findInvoiceFull(client: SupabaseClient, tenantId: string, id: string) {
    ...
    const { data: payments } = await client.from('payments').select('*, received:received_by ( full_name )')
      .eq('invoice_id', id).order('received_at', { ascending: false });

    const { data: refunds } = await client.from('refunds').select('*, refunded:refunded_by ( full_name )')
      .eq('invoice_id', id).order('refunded_at', { ascending: false });
