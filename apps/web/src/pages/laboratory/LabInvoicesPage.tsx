...
import { useLabInvoices } from '@/hooks/useBilling';

const statusTone: Record<string, BadgeTone> = {
  UNPAID: 'red', PARTIAL: 'yellow', PAID: 'green', REFUNDED: 'gray', CANCELLED: 'gray',
};

export function LabInvoicesPage() {
  const [page, setPage] = useState(1);
  const list = useLabInvoices({ page, pageSize: 20 });
  ...
  return (
    <>
      <PageHeader
        title="Lab Invoices"
        subtitle="Lab test billing records â€” LAB-XXXXX"
      />

      {list.isLoading && <div className="flex justify-center py-12"><Spinner size={28} /></div>}
      {list.isError && <Alert tone="error">Failed to load lab invoices.</Alert>}

      {list.data?.rows.length === 0 && (
        <EmptyState
          title="No lab invoices yet"
          description="Invoices are generated automatically when lab tests are ordered."
        />
      )}
