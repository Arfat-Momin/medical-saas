...
import { AlertTriangle, ArrowLeft, Plus, Save, ShieldAlert, Trash2 } from 'lucide-react';
...
export function EditPrescriptionPage() {
  ...
  async function onSave() {
    ...
    try {
      ...
      // Brief confirmation, then return the admin to where they came from
      // (usually the patient's consultation history). If the tab has no
      // history to go back to, fall back to the patient list.
      setSaved(true);
      setTimeout(() => {
  ...
  return (
    <>
      ...
      {saved && <div className="mb-4"><Alert tone="success">Prescription updated. Returning…</Alert></div>}
