...
export const encountersRepository = {
  ...
  /**
   * Write server-side changes back to the local Dexie row, keyed by server_id.
   * Used by admin edit and other post-hoc corrections so the regular
   * ConsultationPage reads refreshed data.
   */
  async applyServerPatch(
    serverId: string,
    patch: Partial<LocalEncounter>,
  ): Promise<void> {
    const row = await db.encounters.where('server_id').equals(serverId).first();
    if (!row) return;
    const now = new Date().toISOString();
    await db.encounters.update(row.local_id, {
      ...patch,
