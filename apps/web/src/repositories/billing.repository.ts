...
export const billingRepository = {
  ...
  createInvoice: (input: CreateInvoiceInput) => api.post<{ invoiceId: string; invoiceNo: string; totalAmount: number }>('/billing/invoices', input),
  ...
  recordPayment: (invoiceId: string, input: { amount: number; method: string; reference?: string | null; notes?: string | null }) =>
    api.post<{ paymentId: string; paidAmount: number; balanceAmount: number; status: string }>(`/billing/invoices/${invoiceId}/payments`, input),
  refundPayment: (invoiceId: string, input: { paymentId: string; amount: number; reason: string }) =>
    api.post<{ refundId: string; status: string; balanceAmount: number }>(`/billing/invoices/${invoiceId}/refunds`, input),
};
